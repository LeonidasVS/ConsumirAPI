...
function ObtenerListado() {
  ...
  const mostrarData = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      <td><button type="button" class="btn btn-danger btnEliminar ">D <i class="bi bi-trash3"></i></button></td>
      <td><button id="update" class="btn btn-primary btnEditar" type="button">U <i class="bi bi-pencil-square"></i></button></td>
      </tr>
      `;
    }
...
function buscarProducto() {
  ...
  fetch(`${API_URL}/api/Obtenerporid/${id}`)
    .then((response) => response.json())
    .then((data) => mostrarData(data))
    .catch((error) => alert("El ID no existe"+error));
...
if (botonActualizarExiste) {
  document
    ...
    .addEventListener("click", function () {
      ...
      validar=validarCampos(nombrepro, preciopro, stockpro);
